...
// The TurtleFunc type is an adapter to allow the use of ordinary
// function as Turtle mutator.
type TurtleFunc func(context.Context, *ent.TurtleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TurtleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TurtleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TurtleMutation", m)
}
